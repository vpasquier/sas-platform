...
export const loadEnvironmentVariablesFile = (mode: "dev" | "prod", envDirPath = path.join(process.cwd(), "env")) => {
  const baseYaml = fs.readFileSync(path.join(envDirPath, ".base.yaml"), "utf-8");
  const modeYaml = fs.readFileSync(path.join(envDirPath, mode === "prod" ? ".prod.yaml" : ".dev.yaml"), "utf-8");
